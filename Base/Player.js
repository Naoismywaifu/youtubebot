...
class Player {
    ...
    constructor(client) {
        ...
        this.manager.on("ready", node => {
            ...
            let isPremiumNode = node.id.startsWith("Premium") ? true : false
    ...
    async handleVideo(message, voiceChannel, song) {
        ...
        if (!serverQueue) {
            ...
            if(Array.isArray(song)){
                message.channel.send(`Playlist detected ! Importing **${song.length}** videos, please wait...`)
    ...
    async getSongs(query) {
       ...
       let res = Rest.load(this.manager.nodes.get(`${node.id}`), `ytsearch: ${query}`)
      /*
